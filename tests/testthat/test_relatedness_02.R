...
test_that("asTasselDistanceMatrix handles special matrix cases", {
    ...
    expect_error(asTasselDistanceMatrix(empty_mat), "Matrix object must have column and row names")
    ...
    expect_no_error(asTasselDistanceMatrix(mat_with_na))

    # Test non-symmetric matrix
    non_sym <- matrix(runif(9), nrow=3)
    rownames(non_sym) <- colnames(non_sym) <- letters[1:3]
    expect_no_error(asTasselDistanceMatrix(non_sym))
})
...
test_that("TasselDistanceMatrix methods work correctly", {
    ...
    expect_equal(round(mat_back, 4), round(test_mat, 4))
})
