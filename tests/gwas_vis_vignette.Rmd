...
data_gwasPoly <- read.GWASpoly(ploidy = 2, pheno.file = "~/Downloads/GWASpoly_download/maizePhenotypes_GWASpoly.txt", geno.file = "~/Downloads/GWASpoly_download/maizeGenotypes_GWASpoly.txt", format = "numeric", n.traits = 3, delim = "\t")
...
head(data_gwasPoly_res@scores$EarDia)

head(data_gwasPoly_res@map)

#issue with gwasPoly output. Effect sizes of QTL don't match the self-contained effect sizes
#data_gwasPoly_res@effects$EarDia[rownames(data_gwasPoly_res@effects$EarDia) %in% c("dummy-1",as.character(get.QTL(data = data_gwasPoly_res)$Marker)),]
