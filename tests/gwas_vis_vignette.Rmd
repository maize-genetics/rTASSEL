...
multiple_trait_manhattan <-function(traitGWASresults){
...
Annotating SNPs with their closest gene
```{r}
genesNearestToGWASranges <- nearest(traitGWASresultsGR, maizeGFFgenesGR, ignore.strand = T)

distToNearest <- distanceToNearest(traitGWASresultsGR, maizeGFFgenesGR, ignore.strand = T)

genesNearestToGWASranges <- maizeGFFgenesGR[genesNearestToGWASranges]

genesNearestToGWASranges <- as.data.frame(genesNearestToGWASranges, row.names = NULL)

genesNearestToGWASranges$distanceToNearestAnnot <- distToNearest@elementMetadata$distance

traitGWASresults_temp <- traitGWASresults

colnames(traitGWASresults_temp) <- paste(colnames(traitGWASresults), "gwas", sep = "_")

genesNearestToGWASranges <- cbind(traitGWASresults_temp, genesNearestToGWASranges)

rm(traitGWASresults_temp)

genesNearestToGWASranges <- makeGRangesFromDataFrame(genesNearestToGWASranges, keep.extra.columns = T)

genesNearestToGWASranges
```
...
genesNearestToGWASranges$Gene_label <- as.character(genesNearestToGWASranges$Gene)
genesNearestToGWASranges$Gene_label[genesNearestToGWASranges$markerLogPVal_gwas < genesNearestToGWASranges$sigTreshold_gwas] <- ""
genesNearestToGWASranges$Gene_label <- as.factor(genesNearestToGWASranges$Gene_label)
summary(genesNearestToGWASranges$Gene_label)

genesNearestToGWASranges$distanceToNearestAnnot_label <- genesNearestToGWASranges$distanceToNearestAnnot
genesNearestToGWASranges$distanceToNearestAnnot_label[genesNearestToGWASranges$markerLogPVal_gwas < genesNearestToGWASranges$sigTreshold_gwas] <- ""

ggplot(genesNearestToGWASranges) + geom_point(aes(Position_gwas, markerLogPVal_gwas, col = trait_gwas), alpha = 0.3) + geom_point(aes(Position_gwas, markerLogPVal_gwas, col = trait_gwas), data = genesNearestToGWASranges[genesNearestToGWASranges$markerLogPVal_gwas > genesNearestToGWASranges$sigTreshold_gwas,])  + geom_hline(aes(yintercept = sigTreshold_gwas, col = trait_gwas), lty = "dashed") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_color_brewer(palette = "Set2") + xlab("Position by Chr") + ylab("-Log10Pval") + geom_label_repel(aes(x = Position_gwas, y = markerLogPVal_gwas, label = Gene_label), inherit.aes = T, box.padding = 2) + geom_label_repel(aes(x = Position_gwas, y = markerLogPVal_gwas, label = distanceToNearestAnnot_label), inherit.aes = T, box.padding = 2)  + facet_wrap(~seqnames, scales = "free_x")

genesNearestToGWASranges$single_label <- paste(genesNearestToGWASranges$Gene_label, genesNearestToGWASranges$distanceToNearestAnnot_label, sep = ": ")

genesNearestToGWASranges$single_label <- gsub("^: $", "" ,genesNearestToGWASranges$single_label)

ggplot(genesNearestToGWASranges) + geom_point(aes(Position_gwas, markerLogPVal_gwas, col = trait_gwas), alpha = 0.3) + geom_point(aes(Position_gwas, markerLogPVal_gwas, col = trait_gwas), data = genesNearestToGWASranges[genesNearestToGWASranges$markerLogPVal_gwas > genesNearestToGWASranges$sigTreshold_gwas,])  + geom_hline(aes(yintercept = sigTreshold_gwas, col = trait_gwas), lty = "dashed") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_color_brewer(palette = "Set2") + xlab("Position by Chr") + ylab("-Log10Pval") + geom_label_repel(aes(x = Position_gwas, y = markerLogPVal_gwas, label = single_label), inherit.aes = T, box.padding = 2) + facet_wrap(~seqnames, scales = "free_x")

#+ geom_rect(aes(xmin = seqRangeStart, xmax = seqRangeEnd, ymin = 0, ymax=1), fill = "black", data=annotationsDF) + geom_rect(aes(xmin=annotationStart, xmax=annotationEnd, ymin=annotationNum, ymax=annotationNum+1, fill = annotationName), data = annotationsDF) 
...
Subset GWAS hits by significance for plotting with genes. Working to make multiple genes show up right. Temp work not working
```{r, eval=F}
traitGWASresultsGR_sig <- traitGWASresultsGR[traitGWASresultsGR$markerLogPVal > traitGWASresultsGR$sigTreshold/1.5,]
...
bestHitByAnnot <- data.frame()
for(annotationName in unique(traitGWASresultsGR_annotations_df$annotationsDF_GR.annotationName)){
  message(annotationName)
  annotationData <- traitGWASresultsGR_annotations_df[traitGWASresultsGR_annotations_df$annotationsDF_GR.annotationName == annotationName,]
  bestHit <- min(annotationData$traitGWASresultsGR.markerpVal)
  message(bestHit)
  bestHitByAnnot <- rbind(bestHitByAnnot, data.frame(annotationsDF_GR.annotationName = annotationName, bestHitPval = bestHit))  
}

traitGWASresultsGR_annotations_df <- merge(traitGWASresultsGR_annotations_df, bestHitByAnnot, by = "annotationsDF_GR.annotationName")

traitGWASresultsGR_annotations_df$annotationsDF_GR.annotationName_label <- as.character(traitGWASresultsGR_annotations_df$annotationsDF_GR.annotationName)

traitGWASresultsGR_annotations_df$annotationsDF_GR.annotationName_label[traitGWASresultsGR_annotations_df$bestHitPval != traitGWASresultsGR_annotations_df$traitGWASresultsGR.markerpVal] <- ""

traitGWASresultsGR_annotations_df$annotationsDF_GR.annotationName_label <- as.factor(traitGWASresultsGR_annotations_df$annotationsDF_GR.annotationName_label)

summary(traitGWASresultsGR_annotations_df$annotationsDF_GR.annotationName_label)

traitGWASresultsGR_annotations_df$MinDistToAnnotation_label <- paste(traitGWASresultsGR_annotations_df$MinDistToAnnotation, "bp")


traitGWASresultsGR_annotations_df$MinDistToAnnotation_label[traitGWASresultsGR_annotations_df$bestHitPval != traitGWASresultsGR_annotations_df$traitGWASresultsGR.markerpVal] <- ""

#ggplot(subset(traitGWASresults, Chrom %in% traitGWASresultsGR_annotations_df$traitGWASresultsGR.seqnames)) + geom_point(aes(Position, markerLogPVal, col = trait), alpha=0.3) + geom_point(aes(Position, markerLogPVal, col = trait), data = subset(traitGWASresults[traitGWASresults$markerLogPVal>traitGWASresults$sigTreshold/1.5,], Chrom %in% traitGWASresultsGR_annotations_df$traitGWASresultsGR.seqnames))  + geom_hline(aes(yintercept = sigTreshold, col = trait), lty = "dashed") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_color_brewer(palette = "Set2") + xlab("Position by Chr") + ylab("-Log10Pval") + geom_label_repel(aes(x = traitGWASresultsGR.start, y = traitGWASresultsGR.markerLogPVal, label = annotationName), data = subset(traitGWASresultsGR_annotations_df, traitGWASresultsGR.markerLogPVal>traitGWASresultsGR.sigTreshold/1.5), inherit.aes = T, box.padding = 2) + geom_label_repel(aes(x = traitGWASresultsGR.start, y = traitGWASresultsGR.markerLogPVal, label = MinDistToAnnotation), data = subset(traitGWASresultsGR_annotations_df, traitGWASresultsGR.markerLogPVal > traitGWASresultsGR.sigTreshold/1.5), inherit.aes = T, box.padding = 2)  + facet_grid(~Chrom, scales = "free_x")

ggplot(traitGWASresultsGR_annotations_df) + geom_point(aes(traitGWASresultsGR.start, traitGWASresultsGR.markerLogPVal, col = traitGWASresultsGR.trait), alpha = 0.3) + geom_point(aes(traitGWASresultsGR.start, traitGWASresultsGR.markerLogPVal, col = traitGWASresultsGR.trait), data = traitGWASresultsGR_annotations_df[traitGWASresultsGR_annotations_df$traitGWASresultsGR.markerLogPVal > traitGWASresultsGR_annotations_df$traitGWASresultsGR.sigTreshold/1.5,])  + geom_hline(aes(yintercept = traitGWASresultsGR.sigTreshold, col = traitGWASresultsGR.trait), lty = "dashed") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_color_brewer(palette = "Set2") + xlab("Position by Chr") + ylab("-Log10Pval") + geom_label_repel(aes(x = traitGWASresultsGR.start, y = traitGWASresultsGR.markerLogPVal, label = annotationsDF_GR.annotationName_label), inherit.aes = T, box.padding = 2) + geom_label_repel(aes(x = traitGWASresultsGR.start, y = traitGWASresultsGR.markerLogPVal, label = MinDistToAnnotation_label), inherit.aes = T, box.padding = 2)  + facet_grid(~traitGWASresultsGR.seqnames, scales = "free_x")
