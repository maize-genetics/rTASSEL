...
##requires annotated gwas object
multiple_trait_manhattan_annot <- function(annotatedGWASresults, label = "annotationName"){
  annotatedGWASresults$Gene_label <- as.character(annotatedGWASresults$Gene)
  
  annotatedGWASresults$Gene_label[annotatedGWASresults$markerLogPVal_gwas < annotatedGWASresults$sigTreshold_gwas] <- ""
  annotatedGWASresults$Gene_label <- as.factor(annotatedGWASresults$Gene_label)
  summary(annotatedGWASresults$Gene_label)
  
  annotatedGWASresults$distanceToNearestAnnot_label <- annotatedGWASresults$distanceToNearestAnnot
  annotatedGWASresults$distanceToNearestAnnot_label[annotatedGWASresults$markerLogPVal_gwas < annotatedGWASresults$sigTreshold_gwas] <- ""
  
  if(label == "annotationName"){
    ggplot(annotatedGWASresults) + geom_point(aes(Position_gwas, markerLogPVal_gwas, col = trait_gwas), alpha = 0.3) + geom_point(aes(Position_gwas, markerLogPVal_gwas, col = trait_gwas), data = annotatedGWASresults[annotatedGWASresults$markerLogPVal_gwas > annotatedGWASresults$sigTreshold_gwas,])  + geom_hline(aes(yintercept = sigTreshold_gwas, col = trait_gwas), lty = "dashed") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + scale_color_brewer(palette = "Set2") + xlab("Position by Chr") + ylab("-Log10Pval") + geom_label_repel(aes(x = Position_gwas, y = markerLogPVal_gwas, label = Gene_label), inherit.aes = T, box.padding = 2) + geom_label_repel(aes(x = Position_gwas, y = markerLogPVal_gwas, label = distanceToNearestAnnot_label), inherit.aes = T, box.padding = 2)  + facet_wrap(~seqnames, scales = "free_x")
  }
  
  if(label == "composite"){
  annotatedGWASresults$single_label <- paste(annotatedGWASresults$Gene_label, annotatedGWASresults$distanceToNearestAnnot_label, sep = ": ")
  annotatedGWASresults$single_label <- gsub("^: $", "" ,annotatedGWASresults$single_label)
...
multiple_trait_manhattan_annot(annotatedGWASresults = genesNearestToGWASranges, label = "composite")
