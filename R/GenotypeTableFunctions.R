...
GWASpolyGenoFromSummarizedExperiment <- function(SummarizedExperimentObject) {
    geno <- data.frame(
        markerName = paste(
            "dummy",
            1:length(ranges(SummarizedExperimentObject@rowRanges)),
            sep = "-"
        ), # dummy name as current summarizeExperimentFromGenotypeTable doesn't keep
        chr = seqnames(SummarizedExperimentObject@rowRanges),
        pos = start(ranges(SummarizedExperimentObject@rowRanges)),
        # same as assay(SummarizedExperimentObject)
        as.data.frame(SummarizedExperimentObject@assays$data@listData)
    )
...
snpMatrixFromGenotypeTable <- function(genotypeTable) {
    ...
    rownames(aMatrix) <- paste0(
        seqnames(genomicRangesDF),
        ":",
