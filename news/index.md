...
- Bug fixes for
  ...
  - Fixed error “missing value where TRUE/FALSE needed” when filtering
    by position with matching start/end chromosomes and NULL position
    values
...
- Added
  ...
  function to download TASSEL JARs from Maven Central and cache them
  locally
- Added JAR resolution system with priority: user option, Maven cache,
  then bundled (legacy fallback)
- Replaced all `class() ==` checks with
  [`inherits()`](https://rdrr.io/r/base/class.html) for proper S4 class
  handling
...
- Updated
  ...
  method dispatch for all `AssociationResults` objects:
  - Will now return default statistics output for all association
    results when running `tableReport(assocObj)` where `assocObj` is an
    object of type `AssociationResults`
...
- Add new function
  ...
  - Allows for grouping from generated hierarchical clustering or
    grouping from metadata via the `metadata` parameter and subsequent
    `mCol` parameters.
...
- `TasselGenotypePhenotype` objects containing genotype table data can
  now be coerced into R `matrix` objects using the function
  [`as.matrix()`](https://rdrr.io/r/base/matrix.html)
  - This will return a taxa x site matrix where taxa is the number of
    rows and sites is the number of columns.
- Added generalized join methods:
  ...
  - Joins phenotype data data based on taxa ID - similar to the TASSEL
    API
...
- No significant updates in this version. This version is virtually
  identical to `0.9.26` and is for linking to Zenodo for archival
  purposes.
...
- [`pca()`](https://rtassel.maizegenetics.net/reference/pca.md) can
  optionally report eigenvalues and eigenvectors as a list object.
- Added new function:
...
- Added new function:
  - [`pca()`](https://rtassel.maizegenetics.net/reference/pca.md)
  - Allows for user to run PCA on rTASSEL objects containing a
    `GenotypeTable` object.
- Added new function:
...
- Added new `TasselDistanceMatrix` class
  ...
  - Prevents console overload and freezing as seen with large distance
    matrix objects.
  - Now shows summary overview of matrix instead of Java object
    reference.
  - Generic functions
    ...
    [`nrow()`](https://rdrr.io/r/base/nrow.html) will return relative
    information similar to how these operate with `matrix` type objects.
  - Primitive function
    [`as.matrix()`](https://rdrr.io/r/base/matrix.html) now supersedes
    deprecated functions `kinshipToRMatrix()` and `distanceToRMatrix()`.
  - Prior functions that take in a kinship object will now take in this
    new class.
- Added new function:
  - [`readTasselDistanceMatrix()`](https://rtassel.maizegenetics.net/reference/readTasselDistanceMatrix.md)
  - Allows for user to read in delimited distance matrix stored in a
    flat file.
- Added new function:
  - [`asTasselDistanceMatrix()`](https://rtassel.maizegenetics.net/reference/asTasselDistanceMatrix.md)
  - Coerces a pairwise matrix (e.g. m x m dimensions) with the same
    column and row names of type `matrix` to an object of type
...
- Added better descriptive error handling for
  [`filterGenotypeTableSites()`](https://rtassel.maizegenetics.net/reference/filterGenotypeTableSites.md)
- Fixed `siteRangeFilterType` parameter bug in
  [`filterGenotypeTableSites()`](https://rtassel.maizegenetics.net/reference/filterGenotypeTableSites.md).
  Now defaults to `none` when user does not specify filter type.
- Added two new parameters to
  [`getSumExpFromGenotypeTable()`](https://rtassel.maizegenetics.net/reference/getSumExpFromGenotypeTable.md)
  - `coerceDosageToInt`: Returns raw byte dosage array instead of
    integer from Java.
  - `verbose`: Display console messages for large “memory-intensive”
    datasets.
...
- Added new filtration features for genotype tables via
...
- Added error checks for catching C stack usage errors for the following
  functions:
  - filterGenotypeTableSites()
