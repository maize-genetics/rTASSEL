...
information, it needs a second table describing what each column is. First,
we will construct an R data frame:
...
The attribute table maps each column of the data frame (`col_id`) to its
TASSEL type (`tassel_attr`), which is one of `taxa`, `data`, `covariate`, or
`factor`:
...
# Load into rTASSEL `TasselPhenotype` object
tasPhenoDF <- readPhenotype(phenoDF, attr = attrDF)
...
a data frame needs its attribute table, which is passed through the `attr`
parameter:
