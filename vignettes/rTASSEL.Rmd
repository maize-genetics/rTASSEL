...
Before using `rTASSEL` for the first time, you must download the TASSEL Java 
libraries. This is a **one-time** setup step that caches the required JAR 
...
Once the JARs are cached, subsequent calls to `library(rTASSEL)` will 
automatically detect and use them. For more details on the caching system, 
custom JAR paths, and troubleshooting, see the 
[Installing rTASSEL](rtassel_installation.html) vignette.
...
create a formal `TasselGenotype` class object in R using the `readGenotype()`
function:

```{r, eval=TRUE, echo=TRUE}
# Load in hapmap file
tasGenoHMP <- readGenotype(genoPathHMP)

# Load in VCF file
tasGenoVCF <- readGenotype(genoPathVCF)
```

`readGenotype()` inspects what it is given, so the same function reads every
supported file type as well as an R matrix of numeric genotype values (see
...
This summary details the number of taxa and marker positions in the data set,
a preview of the genotype calls themselves, and the memory address of the
underlying TASSEL object.

### Additional information about rTASSEL data objects
...
the reference TASSEL object ID. If we wanted to extract the `GenotypeTable`
that a `TasselGenotype` object points at, we would use the `javaRefObj()`
accessor:

```{r, eval=TRUE, echo=TRUE}
tasGenoHMP |> javaRefObj()
```
...
environment. Every function that does so is collected in the
[Extracting Data into R](extracting_data.html) vignette.

### Joining genotype data split across files
Genotype data often arrives split up - one file per chromosome, or one file
per collection of sites. `intersectJoin()` and `unionJoin()` put such files
back together into a single `TasselGenotype` object. For example, the package
ships two small VCF files, one holding sites on chromosome 1 and the other
sites on chromosome 5:

```{r, eval=TRUE, echo=TRUE}
tasGenoChr1 <- readGenotype(
    system.file("extdata", "rt_sub_chr1.vcf", package = "rTASSEL")
)
tasGenoChr5 <- readGenotype(
    system.file("extdata", "rt_sub_chr5.vcf", package = "rTASSEL")
)

tasGenoChrAll <- intersectJoin(tasGenoChr1, tasGenoChr5)
tasGenoChrAll
```

The sites of every table are kept and returned in genomic order, whichever
order the tables were given in:

```{r, eval=TRUE, echo=TRUE}
tasGenoChrAll |> positionList()
```

The two joins differ in which taxa they keep: `intersectJoin()` keeps the
taxa every table holds, while `unionJoin()` keeps every taxon any of them
holds and returns the calls a table never made as missing.

Joining is for tables that hold *different* sites. To merge the calls of
tables that describe the same sites, use `mergeGenotypeTables()` instead.
...
Here, we will use the `readPhenotype()` function:
...
# Load into rTASSEL `TasselPhenotype` object
tasPheno <- readPhenotype(phenoPath)

# Inspect object
tasPheno
```

The object output shows the trait data itself along with the TASSEL attribute
type of each column.

### From an R data frame
In some cases you might want to first modify your phenotype data set in R and
then load it into the TASSEL environment. `readPhenotype()` also accepts a
`data.frame`, but since a plain data frame carries no TASSEL attribute
information, the `attrTypes` parameter supplies them. First, we will construct
an R data frame:
...
In the simplest form, `attrTypes` is a named character vector: the names are
the columns of the data frame, and the values their TASSEL types, which are
one of `taxa`, `data`, `covariate`, or `factor`:

```{r, eval=TRUE, echo=TRUE}
# Load into rTASSEL `TasselPhenotype` object
tasPhenoDF <- readPhenotype(
    phenoDF,
    attrTypes = c(
        Taxon  = "taxa",
        EarHT  = "data",
...
`attrTypes` plays the part `colData` plays in a `SummarizedExperiment`: it
has to describe every column of the data frame exactly once, and describe
nothing else. A column left out, named twice, or misspelled is an error
rather than a silently dropped trait, and each type accepts only the R types
it can hold - a `character` column marked as `data`, or a column of dates,
is reported by name instead of being coerced on a guess.

The mapping can also be written as a data frame, which is easier to build
programmatically. Its columns are `col_id` and `tassel_attr`:

```{r, eval=TRUE, echo=TRUE}
# Describe each column
attrDF <- tibble::tibble(
    col_id      = colnames(phenoDF),
    tassel_attr = c("taxa", rep("data", ncol(phenoDF) - 1))
)

attrDF

readPhenotype(phenoDF, attrTypes = attrDF)
```


## Filtering phenotype data

**NOTE**: This is just a "snapshot" of how we can filter phenotype
information in rTASSEL. For more information, please see the additional
vignette, *"Filtering Phenotype Data"*.

Phenotype objects are filtered with the same pipe-friendly verbs used on
genotype tables. A phenotype has its own two axes - observations and traits -
so the verbs address those instead:
...
To show these off we will use a phenotype file that carries all three
non-taxa TASSEL attribute types - a `factor`, three `data` traits, and three
`covariate` traits:

```{r, eval=TRUE, echo=TRUE}
tasPhenoFull <- readPhenotype(
    system.file("extdata", "mdp_phenotype.txt", package = "rTASSEL")
)
tasPhenoFull
```

`filterTaxa()` keeps observations, with every trait column in scope. Several
expressions are combined with `&`, as in `dplyr::filter()`:

```{r, eval=TRUE, echo=TRUE}
tasPhenoFull |> filterTaxa(location == "A", EarHT > 100)
```

On the other axis, `selectTraits()` takes trait names or any `tidyselect`
expression, and `filterTraits()` describes traits by their metadata, such as
their TASSEL attribute type:

```{r, eval=TRUE, echo=TRUE}
tasPhenoFull |> selectTraits(EarHT, starts_with("Q"))

tasPhenoFull |> filterTraits(traitType == "covariate")
```

The taxa column is an axis rather than a trait, so it is never dropped by the
trait verbs. As with genotype data, every verb returns a new object of the
same class it was given, leaving the original alone.
...
can use the `readGenomicDataset()` function, which joins the two on their
shared taxa and returns a `TasselGenomicDataset` object. For genotype input,
the following can be used:

* a path to a genotype file
* an R matrix of numeric genotype values
* a prior `TasselGenotype` object

For phenotype input, the following can be used:

* a path to a phenotype data set
* an R data frame (along with its attribute table)
* a prior `TasselPhenotype` object

For example, if we wanted to join the genotype and phenotype objects from
earlier:

```{r, eval=TRUE, echo=TRUE}
tasGenoPheno <- readGenomicDataset(
    genotype  = tasGenoHMP,
    phenotype = tasPheno
)
tasGenoPheno
...
genotype path and a phenotype data frame, etc.). As with `readPhenotype()`,
a data frame needs its attribute types, which are passed through the
`attrTypes` parameter:

```{r, eval=TRUE, echo=TRUE}
tasGenoPhenoDF <- readGenomicDataset(
    genotype  = genoPathHMP,
    phenotype = phenoDF,
    attrTypes = attrDF
)
tasGenoPhenoDF
...
By default, taxa are combined with an intersect join, keeping only the taxa
found in both data sets. Use `join = "union"` to keep taxa found in either.

The two components of a dataset can be pulled back out at any time with
`genotype()` and `phenotype()`:

```{r, eval=TRUE, echo=TRUE}
tasGenoPheno |> genotype()
```
...
Like other genotype files, we can read this in via `readGenotype()`:

```{r, eval=TRUE, echo=TRUE}
numGtPath <- system.file("extdata", "numeric_genotype.txt", package = "rTASSEL")
numGt <- readGenotype(numGtPath)
...
Once properly formatted, the matrix can be handed to `readGenotype()`, which
returns a `TasselNumericGenotype` object:

```{r, eval=TRUE, echo=TRUE}
numGt <- readGenotype(simMat)
...
**NOTE**: This is just a "snapshot" of how we can pull data out of rTASSEL
objects. For the full set of accessors and coercion methods, please see the
additional vignette, *"Extracting Data into R"*.
...
base R's `as.data.frame()` function on any object *containing a phenotype
table* (a `TasselPhenotype` or a `TasselGenomicDataset`):

```{r, eval=TRUE, echo=TRUE}
tasExportPhenoDF <- as.data.frame(tasGenoPheno)
...
In `rTASSEL`, both are reached with the same bracket (`[`) syntax used to
subset a `matrix` or `data.frame`, where rows select taxa and columns select
marker sites:
...
Criteria are described with selector functions - `taxa()` and `taxaWhere()`
for rows; `sites()`, `siteIds()`, `chrom()`, `region()`, and `sitesWhere()`
for columns. For example, in our genotype data set, if we want to remove
monomorphic and low coverage sites, we could use `sitesWhere()`:

```{r, eval=TRUE, echo=TRUE}
tasGenoPhenoFilt <- tasGenoPheno[, sitesWhere(alleleCount >= 150 & maf >= 0.05)]
...
Taxa are filtered the same way, and both axes can be given at once:

```{r, eval=TRUE, echo=TRUE}
tasGenoPheno[taxaWhere(notMissing >= 0.9), sitesWhere(maf >= 0.05)]
```

Filtering returns the class it was given, so a `TasselGenotype` yields a
`TasselGenotype` and a `TasselGenomicDataset` yields a
`TasselGenomicDataset` with its phenotype data re-joined against the taxa
that survived.
...
parameter input is any `rTASSEL` object that contains a genotype data set:
...
This object, similar to the genotype and phenotype classes, essentially holds
...
function on any `rTASSEL` object that contains a TASSEL `GenotypeTable`
...
Both of these return a results object holding a collection of TASSEL 5 table
reports, which `tableReport()` reads. The default report is a table with a
Taxa ID column and the number of components or axes that were specified in
the function call. For example, let's take a look at the `pcaRes` object made
previously:
...
* `tasObj`: a `TasselPhenotype` or `TasselGenomicDataset` R object
...
tasPhenoCov <- readPhenotype(phenoPathCov)
...
### Using BLUEs as phenotype data
The estimates themselves are one value per taxon per trait, which is how
TASSEL models a phenotype. An `AssociationResultsBLUE` object can therefore
be handed to the join functions, in the same way a `TasselPhenotype` object
can, so that the estimates become the traits of a new data set:

```{r, echo=TRUE, eval=TRUE}
# Population structure to control for in the next model
popStructPath <- system.file(
    "extdata",
    "mdp_population_structure.txt",
    package = "rTASSEL"
)
tasPopStruct <- readPhenotype(popStructPath)

# Join the estimates, the covariates, and a genotype table
tasBlueGenoPheno <- intersectJoin(tasGenoHMP, tasBLUE, tasPopStruct)
tasBlueGenoPheno
```

Since the field variation the factors captured has been absorbed into the
estimates, the result is ready to fit markers against:

```{r, echo=TRUE, eval=TRUE}
assocModelFitter(
    tasObj     = tasBlueGenoPheno,
    formula    = . ~ .,
    fitMarkers = TRUE
) |> tableReport() |> head()
```
...
combined with our phenotype data in a `TasselGenomicDataset` object:
...
tasGenoPhenoFast <- readGenomicDataset(
    genotype  = tasGenoHMP,
    phenotype = phenoPathFast
)
...
with any `rTASSEL` object containing genotype table information:
...
**Note:** `createTree()` requires the
[ape](https://cran.r-project.org/web/packages/ape/ape.pdf) package to be
installed (`install.packages("ape")`). Upon creation, the `phyloTree` object
is returned as a `phylo` object generated by ape:
...
the `plot*` family of functions. These include Manhattan plots, QQ plots, PCA 
and scree plots, SNP density plots, and LD heatmaps with block highlighting 
and genomic position tracks. In the following sections, we will briefly go 
over some of these capabilities.
...
## SNP density plots
To visualize the distribution of SNPs across chromosomes in your genotype
data, use the `plotSnpDensity()` function. This generates a heatmap-style
plot showing how many SNPs fall within fixed-size genomic windows across
all chromosomes:
...
Like other `rTASSEL` visualizations, an interactive version can be produced
...
1. **Compute LD** with `linkageDiseq()`, which returns an `LDResults`
   object.
2. **Visualize** with `plotLD()`, which accepts the `LDResults` object
   and produces a `ggplot2`-based LD heatmap.
...
tasGenoPhenoFilt <- tasGenoPheno[, region("2", 228e6, 300e6)]
...
### Basic LD plot
Pass the `LDResults` object to `plotLD()` to generate the LD heatmap:

```{r, echo=TRUE, eval=TRUE}
plotLD(ldObj = myLDResults)
```

By default, `plotLD()` displays $r^{2}$ values. You can change the
displayed statistic using the `plotVal` parameter:
...
You can highlight regions of interest on the LD plot using the `ldBlocks`
parameter. Regions are defined with `LDRegion` objects, which specify
a start and end position (in bp) along with optional labels, colors, and
line widths:
...
* `linewidth` -- outline thickness (mm); defaults to an auto-calculated
  value
* `showSpan` -- if `TRUE` (default), the genomic span (e.g., "15 Mbp")
  is appended to the label annotation


### Genomic track
Setting `genomicTrack = TRUE` draws a horizontal genomic position track
above the LD heatmap. This shows the approximate physical spacing of each
site and connects positions to the evenly-spaced LD triangle below:
