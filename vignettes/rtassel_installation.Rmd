...
If you wish to run a containerized version of `rTASSEL`, use the
[`maizegenetics/rtassel_docker`](https://hub.docker.com/r/maizegenetics/rtassel_docker)
image on Docker Hub.

> **Deprecated:** The Docker Hub image
> [`maizegenetics/rtassel`](https://hub.docker.com/r/maizegenetics/rtassel) is
> **deprecated**. Do not use `docker pull maizegenetics/rtassel` or
> `docker run ... maizegenetics/rtassel`; switch any scripts or notes to
> [`maizegenetics/rtassel_docker`](https://hub.docker.com/r/maizegenetics/rtassel_docker)
> (see below).

For reproducibility, pull a **version tag** that matches your rTASSEL release
(for example `v0.12.0`). The `latest` tag tracks the most recently published
release build.

```
docker pull maizegenetics/rtassel_docker:v0.12.0
```

The [`docker/Dockerfile`](https://github.com/maize-genetics/rTASSEL/blob/master/docker/Dockerfile)
in the rTASSEL repository defines this image; you can build locally with
`docker build --build-arg RTASSEL_TAG=v0.12.0 -f docker/Dockerfile .` from a
clone checked out at that tag.
...
```
docker run --rm -ti maizegenetics/rtassel_docker:v0.12.0 R
```
...
```
docker run --rm -ti -p 8787:8787 maizegenetics/rtassel_docker:v0.12.0
```
