...
```{r}
genoPhenoCombined <- combineTasselGenotypePhenotype(siteAndTaxaFiltGenoTable@jtsGenotypeTable,
                                                    jtsPhenotype)
...
filtGLMresults <- filtertopSNPS(LMoutput = GMLresults, 
                                  ...
                                  sortByTrait = TRUE)
topSNPgenoSample <- SNPintersection(GenotypeTable = 
...
## Run MLM on Top SNPs
Now take the topSNPgenoSample and the filtAndIntersectPhenoTable and run the MLM on them

```{r}
topSNPsMLMresults <- tasselMLM(Genotypes = topSNPgenoSample,
                               Phenotypes = filtAndIntersectPhenoTable)
```
