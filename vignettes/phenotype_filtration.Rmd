...
## The two grammars
Phenotype data is also filtered with the same bracket (`[`) grammar used on
genotype tables, built from **selector** functions. Where a genotype table
is indexed `gt[<taxa>, <sites>]`, a phenotype is indexed
`ph[<observations>, <traits>]`:

| Selector        | Verb equivalent  | Selects observations/traits by  |
|-----------------|------------------|---------------------------------|
| `taxaWhere()`   | `filterTaxa()`   | Expressions over trait columns  |
| `taxa()`        | `selectTaxa()`   | Taxa IDs                        |
| numeric `i`     | -                | (not available; use `sliceTaxa()`) |
| `traitsWhere()` | `filterTraits()` | Expressions over trait metadata |
| `traits()`      | `selectTraits()` | Trait names                     |
| numeric `j`     | `sliceTraits()`  | Trait position (1-based)        |

The row selectors are the ones you already use on genotype tables, and
`traits()` and `traitsWhere()` are new for the trait axis. A bare character
vector works in either index, naming taxa in `i` and traits in `j`, and any
selector can be inverted with `!`. Leave an index out to keep everything on
that axis.

Both grammars are backed by the same machinery, so either style gives the
same result:
...
The rest of this vignette shows both spellings side by side. Which to reach
for is a matter of taste, with two things to note. The verbs take the whole
`tidyselect` vocabulary, so pattern matching on names is theirs alone; and
brackets filter both axes in one expression, where verbs are piped one after
another.
...
## Overview
Row criteria go in the first bracket index, or to a `*Taxa()` verb.
`taxaWhere()` and `filterTaxa()` evaluate an R expression against the
phenotype's own columns, one element per observation, so these are
...
Whole taxa are covered by `taxa()` and `selectTaxa()` (by ID), and by
`sliceTaxa()` (by position).
...
A range is expressed with two comparisons. `taxaWhere()` takes one
expression, so they are joined with `&`, while `filterTaxa()` takes them as
separate arguments and joins them for you, exactly as `dplyr::filter()`
does:
...
## ...using taxa IDs
Whole taxa are selected with `taxa()` or `selectTaxa()`, which take literal
IDs. Every observation of a selected taxon is kept, so here two taxa give
four rows:
...
A bare character vector means the same thing in the first index, so
`myPheno[c("33-16", "38-11"), ]` is another way to write it.
...
picked out by pattern. This is the one thing `taxa()` cannot do, since the
bracket grammar takes literal IDs only:

```{r, eval=TRUE, echo=TRUE}
myPheno |> selectTaxa(starts_with("CML"))
```

Because `taxaId` is in scope for `taxaWhere()` and `filterTaxa()`, the same
selection can be written as a predicate in either grammar. It is equivalent
to the above, since testing the taxon ID either keeps or drops every
...
and starting from 1. Like `selectTaxa()`, it works a taxon at a time. The
row index has no positional spelling, so this one is a verb only:
...
Because a row predicate is a regular R expression, taxa IDs can be matched
with any function that returns a logical vector:
...
## Overview
Column criteria go in the second bracket index, or to a `*Traits()` verb.
There are two ways to describe a trait. `traits()` and `selectTraits()`
work from the trait columns themselves - by name, and in the verb's case by
value too, with the `tidyselect` vocabulary. `traitsWhere()` and
`filterTraits()` instead evaluate an R expression against per-trait
metadata, and these columns are available in that expression:
...
Positional criteria are covered by `sliceTraits()` and by a bare numeric
second index.

The taxa column is the axis the observations sit on rather than a trait, so
it is not counted by any of these: it is never offered to `traitsWhere()`
or `filterTraits()`, cannot be named in `traits()` or `selectTraits()`, and
is not a position in `sliceTraits()` or in `ph[, 1:3]`. Whatever traits are
kept, the taxa column stays with them.
...
## ...using trait names
Traits can be selected by their literal names with `traits()` or
`selectTraits()`:
...
As in the first index, a bare character vector means the same thing, so
`myPheno[, c("EarHT", "dpoll")]` is another way to write it.
...
`dplyr::select()` work here. As with taxa, this is what the verb gives you
over `traits()`, which takes literal names only. `starts_with()`,
...
`traitsWhere()` or `filterTraits()`.


## ...using attribute types
A trait predicate describes traits by their metadata. The `traitType`
column holds each trait's TASSEL attribute type, so the population
structure covariates can be pulled out on their own:
...
A bare numeric second index and `sliceTraits()` keep traits by position,
counted as `traitNames()` reports them and starting from 1. The taxa column
is not a position, so position 1 is the first trait:
...
`traitIndex` inside a trait predicate is the same 1-based position, so
`traitsWhere(traitIndex <= 3)` is another way to write `myPheno[, 1:3]`.


## Inverting a selection
The two grammars negate differently. In brackets, `!` inverts the selector,
so it keeps whatever the selector did not name:
...
The verbs follow `dplyr` instead. `selectTraits()` and `sliceTraits()` drop
with a leading `-`:
...
`filterTaxa(!(location == "A"))` and `myPheno[!taxaWhere(location == "A"), ]`
are the same query as the last one.



# Combining filters

## Observations and traits in one call
Both bracket indexes can be given at once, which filters observations and
traits in a single expression. With verbs, each takes an object and returns
one, so the two axes are filtered by piping one verb into the next:
...
the time, so filtering observations first changes what a trait predicate
...
Brackets apply their indexes left to right, so the same two pipelines are
written with and without a row index:
...
when it reaches the trait filter:
...
the observations first and drop the trait second, which the bracket order
does for you:
...
The trait selectors and verbs act on the phenotype half and leave the
genotype table alone:
...
A dataset has two row axes that a taxa predicate could address - the taxa
of the genotype table and the observations of the phenotype - so both
grammars read the expression to decide which. Naming a phenotype column
filters observations; anything else filters the genotype table, so the
genotype metrics `notMissing` and `het` described in *Filtering Genotype
Tables* keep their meaning throughout:
...
`taxa()`, `selectTaxa()`, and `sliceTaxa()` mean the same thing on either
half, so on a dataset they are read through the genotype table and give the
same result they would on either component alone.

A dataset has two column axes as well, which the second bracket index tells
apart by what it is given: a trait selector addresses the phenotype and
everything else addresses the sites of the genotype table. To subset both,
chain two calls:
...
The phenotype verbs likewise chain with the site verbs, so a dataset can be
trimmed on every axis in one pipeline:
...
Phenotype filtering is genotype filtering applied to a different pair of
...
| Bracket grammar            | `gt[<taxa>, <sites>]`                            | `ph[<observations>, <traits>]`                       |
| Column selectors           | `sites()`, `siteIds()`, `chrom()`, `region()`, `sitesWhere()` | `traits()`, `traitsWhere()`, numeric `j` |
