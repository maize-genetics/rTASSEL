...
vignette: >
  ...
  %\VignetteIndexEntry{GWAS Visualization}
...
summary(tasselPhenotypeFromRDF)
...
summary(blueReports)
summary(blueReports$BLUEs_FromR)
summary(blueReports$Phenotype_ANOVA_FromR)
...
traitGWASresultsTassel <- tasselGWASPluginResDFToManhattanDF(gwasPluginResultDF = gwasReports, pluginName = "fixedEffectLMPlugin")
summary(traitGWASresultsTassel)
ddply(traitGWASresultsTassel, .(Trait), summarize, nMarkers=length(unique(Marker)))
```

Create qq-plots
```{r}
qq.plot(data_gwasPoly_res, trait = "EarHT", model = "additive")

qqPlot_multi_trait(traitGWASresults, fancy = F)

qqPlot_multi_trait(traitGWASresults, fancy = T)
...
head(traitGWASresults_annotated[,-21]) #just hiding column with long IDs 
```
